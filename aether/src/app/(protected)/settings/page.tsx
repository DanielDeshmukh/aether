...
export default function SettingsPage() {
  const [user, setUser] = useState<{ email: string; name: string | null; provider?: string; created_at?: string } | null>(null);
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      await fetch("/api/v1/auth/account", { method: "DELETE", credentials: "same-origin" });
    ...
    return (
      <div className="min-h-screen bg-[#050505] font-lambo text-white">
        <main className="relative px-5 pt-28 md:px-10">
          <div className="chamfer-panel mx-auto max-w-2xl border border-white/10 bg-white/[0.02] p-8 text-[10px] tracking-[0.3em] text-lambo-ash">Loading profile...</div>
        </main>
      </div>
    );
  }
        ...
        <section className="relative mx-auto max-w-2xl space-y-6">
          ...
          <form onSubmit={handleSave} className="chamfer-panel border border-lambo-gold/20 bg-[#0d0d0d] p-8 space-y-6">
            <div>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full border border-white/10 bg-black/40 px-4 py-3 text-sm text-lambo-white focus:border-lambo-gold/40 focus:outline-none" />
            </div>
            <div>
              <label className="text-[10px] font-bold tracking-[0.3em] text-lambo-gold block mb-3">Email</label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full border border-white/10 bg-black/40 px-4 py-3 text-sm text-lambo-white focus:border-lambo-gold/40 focus:outline-none" />
            </div>
            <div>
            ...
            <button type="submit" disabled={isSaving} className="chamfer-button w-full border border-lambo-gold/30 bg-lambo-gold/10 px-4 py-4 text-[10px] font-bold tracking-[0.2em] text-lambo-gold transition-colors hover:bg-lambo-gold/20 disabled:opacity-50">
          ...
          <div className="chamfer-panel border border-white/10 bg-[#0d0d0d] p-8 flex flex-col gap-4 sm:flex-row sm:justify-between">
            <button type="button" onClick={handleSignOut} className="chamfer-button border border-white/10 bg-white/5 px-6 py-3 text-[10px] font-bold tracking-[0.2em] text-lambo-ash transition-colors hover:border-lambo-gold/40 hover:text-lambo-gold">Sign Out</button>
            <button type="button" onClick={handleDeleteAccount} className="chamfer-button border border-red-500/30 bg-red-500/10 px-6 py-3 text-[10px] font-bold tracking-[0.2em] text-red-400 transition-colors hover:bg-red-500/20">Delete Account</button>
          </div>
        </section>
      </main>
    </div>

...
export default function DashboardPage() {
  ...
  const filteredScans = useMemo(() => {
    let result = [...scans];
    if (filterStatus !== "all") result = result.filter((s) => normalizeStatus(s.status) === filterStatus);
    if (sortBy === "date") result.sort((a, b) => new Date(b.created_at ?? 0).getTime() - new Date(a.created_at ?? 0).getTime());
    else if (sortBy === "threat") { const order: Record<string, number> = { critical: 0, high: 1, medium: 2, low: 3, unknown: 4 }; result.sort((a, b) => (order[a.threat_level?.toLowerCase()] ?? 4) - (order[b.threat_level?.toLowerCase()] ?? 4)); }
    else if (sortBy === "status") result.sort((a, b) => normalizeStatus(a.status).localeCompare(normalizeStatus(b.status)));
    return result;
    ...
    <div className="min-h-screen bg-[#050505] font-lambo text-white">
      ...
      <main className="relative overflow-hidden px-5 pb-16 pt-24 md:px-10 md:pt-28">
        ...
        <section className="relative mx-auto max-w-7xl">
          <div className="chamfer-panel border border-lambo-gold/20 bg-[#0d0d0d] p-4 shadow-[0_0_0_1px_rgba(255,255,255,0.02)] sm:p-6 md:p-8 lg:p-10">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-2xl">
                ...
                <p className="mt-4 max-w-xl text-sm tracking-[0.2em] text-lambo-ash">Review persisted scan targets, inspect AI reasoning, and keep the operator loop anchored to the latest engine state.</p>
              </div>
          ...
          {error && <div className="chamfer-panel mt-6 border border-[rgba(255,123,114,0.28)] bg-[rgba(255,59,48,0.08)] px-5 py-4 text-[10px] font-bold tracking-[0.28em] text-[#ff7b72]">{error}</div>}

          <div className="mt-6"><ScanChart scans={scans} /></div>

          <div className="mt-6 flex flex-wrap items-center gap-4 text-[10px] tracking-[0.2em]">
            <select value={filterStatus} onChange={(e) => { setFilterStatus(e.target.value); setPage(1); }} className="chamfer-badge border border-white/10 bg-[#0d0d0d] px-4 py-3 text-lambo-ash focus:border-lambo-gold/40 focus:outline-none">
            ...
            <select value={sortBy} onChange={(e) => { setSortBy(e.target.value); setPage(1); }} className="chamfer-badge border border-white/10 bg-[#0d0d0d] px-4 py-3 text-lambo-ash focus:border-lambo-gold/40 focus:outline-none">
          ...
          {isLoading ? (
            ...
            <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {paginatedScans.map((scan) => {
                ...
                const results = scan.results as Record<string, unknown> | undefined;
                ...
                const auditFindings = auditEngine?.findings as unknown[] | undefined;
                const headerFindings = headerAudit?.findings as unknown[] | undefined;
                const vulnCount = (auditFindings?.length ?? headerFindings?.length ?? 0) as number;
                  ...
                  <article key={scan.id} className="chamfer-panel border border-white/5 bg-[#0d0d0d] p-6 transition-colors duration-300 hover:border-lambo-gold/30">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex flex-col items-end gap-2">
                        ...
                        {scan.is_verified ? (
                          <span className="chamfer-badge border border-green-500/30 bg-green-500/10 px-2 py-1 text-[9px] font-bold tracking-[0.2em] text-green-400">Verified</span>
                        ) : (
                          <span className="chamfer-badge border border-yellow-500/30 bg-yellow-500/10 px-2 py-1 text-[9px] font-bold tracking-[0.2em] text-yellow-400">Unverified</span>
                        )}
                      </div>
                    </div>

                    <div className="mt-4 flex items-center gap-4 text-[10px] tracking-[0.22em] text-lambo-ash">
                      {vulnCount > 0 && <span className="border border-red-500/30 bg-red-500/10 px-2 py-1 text-red-400">{vulnCount} finding{vulnCount !== 1 ? "s" : ""}</span>}
                    ...
                    <div className="mt-4 flex gap-3">
                      <button type="button" onClick={() => router.push(`/dashboard/${scan.id}`)} className="chamfer-button flex-1 border border-lambo-gold/30 bg-lambo-gold/10 px-4 py-3 text-[10px] font-bold tracking-[0.2em] text-lambo-gold transition-colors duration-300 hover:bg-lambo-gold/20">
                        View Debrief
                      </button>
                      {deleteConfirmId === scan.id ? (
                        <div className="flex gap-2">
                          <button type="button" onClick={() => handleDelete(scan.id)} className="chamfer-button border border-red-500/50 bg-red-500/20 px-3 py-3 text-[10px] font-bold tracking-[0.15em] text-red-400 hover:bg-red-500/30 transition-colors">Yes</button>
                          <button type="button" onClick={() => setDeleteConfirmId("")} className="chamfer-button border border-white/10 bg-white/5 px-3 py-3 text-[10px] font-bold tracking-[0.15em] text-lambo-ash hover:bg-white/10 transition-colors">No</button>
                        </div>
                      ) : (
                        <button type="button" onClick={() => setDeleteConfirmId(scan.id)} className="chamfer-button border border-white/10 bg-white/5 px-3 py-3 text-[10px] font-bold tracking-[0.15em] text-lambo-ash hover:border-red-500/30 hover:text-red-400 transition-colors">Delete</button>
                      )}
                    </div>
                  </article>
                );
          ...
          {!isLoading && filteredScans.length > PAGE_SIZE && (
            <div className="mt-8 flex items-center justify-center gap-4">
              <button type="button" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="chamfer-button border border-white/10 bg-white/5 px-4 py-3 text-[10px] font-bold tracking-[0.2em] text-lambo-ash disabled:opacity-30 hover:border-lambo-gold/40 hover:text-lambo-gold transition-colors">Prev</button>
              <span className="text-[10px] tracking-[0.3em] text-lambo-ash">Page {page} of {totalPages}</span>
              <button type="button" onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page >= totalPages} className="chamfer-button border border-white/10 bg-white/5 px-4 py-3 text-[10px] font-bold tracking-[0.2em] text-lambo-ash disabled:opacity-30 hover:border-lambo-gold/40 hover:text-lambo-gold transition-colors">Next</button>
            </div>
          )}

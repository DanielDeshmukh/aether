...
interface InputUrlProps {
  onTerminalStart?: (payload: Record<string, unknown>) => void;
  className?: string;
  consentConfirmed: boolean;
  onConsentChange: (value: boolean) => void;
}

export default function InputUrl({ onTerminalStart, className = "", consentConfirmed, onConsentChange }: InputUrlProps) {
  ...
  const handleFinalExecution = async () => {
    ...
    try {
      const response = await fetch("/api/v1/scans", {
        ...
        body: JSON.stringify({ target_url: url, consent_confirmed: consentConfirmed }),
      });
      ...
      setError((submitError instanceof Error ? submitError.message : "ENGINE HANDSHAKE FAILED.").toUpperCase());
      setStep("confirm");
    } finally {

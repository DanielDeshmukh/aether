...
class TestEmailAndModelFallbacks(unittest.TestCase):
    ...
    def test_model_fallbacks_use_current_ids(self):
        ...
        fake_client.models.generate_content.side_effect = RuntimeError("not found")
        ...
        with (
            ...
            with self.assertRaises(RuntimeError):
                agent._generate_with_retry(fake_client, contents="x", config={}, max_retries=1)

        attempted_models = [
            call.kwargs["model"] for call in fake_client.models.generate_content.call_args_list
        ]
        self.assertEqual(attempted_models, ["gemini-2.5-flash", "gemini-2.5-flash-lite", "gemini-2.5-pro"])

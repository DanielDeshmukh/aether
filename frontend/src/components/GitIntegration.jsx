...
const GitIntegration = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await apiRequest('/api/v1/git-targets', {
        method: 'POST',
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-xs text-lambo-ash">
          Configure a Git provider to automatically create pull requests with remediation fixes.
        </p>
      </div>
            ...
            {targets.map((t) => (
              <div key={t.id} className="flex items-center justify-between border border-white/5 bg-black/20 px-4 py-3">
                ...
                <button onClick={() => handleDelete(t.id)}
                  className="text-[10px] tracking-[0.2em] text-red-400 hover:text-red-300">
                  REMOVE
                </button>
              </div>
            ))}
      ...
      <form onSubmit={handleSubmit} className="chamfer-panel border border-lambo-gold/20 bg-[#0d0d0d] p-8 space-y-5">
        ...
        <div>
          ...
          <select value={form.target_id} onChange={(e) => {
            ...
            {targets.map((t) => (
              <option key={t.id} value={t.id}>{t.domain} ({t.git_provider || 'not configured'})</option>
            ))}
          </select>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div>
            ...
            <select value={form.git_provider} onChange={(e) => setForm({ ...form, git_provider: e.target.value })}
              className="w-full border border-white/10 bg-black/40 px-4 py-3 text-sm text-lambo-white focus:border-lambo-gold/40 focus:outline-none">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div>
            ...
            <input type="text" value={form.default_branch} onChange={(e) => setForm({ ...form, default_branch: e.target.value })}
              className="w-full border border-white/10 bg-black/40 px-4 py-3 text-sm text-lambo-white focus:border-lambo-gold/40 focus:outline-none" />
          </div>
          <div>
            <label className="text-[10px] font-bold tracking-[0.3em] text-lambo-ash block mb-2">Base Branch (PR target)</label>
            <input type="text" value={form.base_branch} onChange={(e) => setForm({ ...form, base_branch: e.target.value })}
        ...
        {form.git_provider === 'gitlab' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <input type="text" value={form.api_base_url} onChange={(e) => setForm({ ...form, api_base_url: e.target.value })}
        ...
        <button type="submit" disabled={saving || !form.target_id || !form.access_token || !form.repository}

    ...
    useEffect(() => {
        ...
        socketRef.current.onclose = () => {
            setStatus((currentStatus) => (
                currentStatus === 'terminated' || currentStatus === 'failed' ? currentStatus : 'completed'
            ));
        };
            ...
            <div className="bg-[#050505] border border-white/10 overflow-hidden">
                <div className="bg-[#0d0d0d] border-b border-white/10 p-4 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] text-lambo-white tracking-[0.3em] font-bold">
                            // Aether_Engine_Core
                        </span>
                    </div>
                ...
                >
                    {!scanSession && (
                        <div className="text-[11px] tracking-[0.15em] text-lambo-ash">
                            Awaiting a target authorization to start the engine loop.
                        </div>
                    )}
                    ...
                    {status === 'scanning' && (
                        <div className="text-lambo-gold text-[11px] animate-pulse py-2">
                            _ Engine active: Streaming telemetry...
                        </div>
                    )}
                    ...
                    {status === 'completed' && (
                        <div className="mt-6 border border-lambo-gold/30 bg-lambo-gold/10 p-5 text-center animate-in fade-in slide-in-from-bottom-2">
                            ...
                            <p className="text-[10px] text-lambo-white tracking-[0.05em] mb-5">
                                The system has finished scanning. Please head towards the dashboard to view the full audit result.
                            </p>
                            <Link 
                                to="/dashboard" 
                                className="inline-block border border-lambo-gold bg-lambo-gold px-6 py-3 text-[10px] font-bold tracking-[0.15em] text-black hover:bg-lambo-gold-dark hover:border-lambo-gold-dark transition-colors"
                            >
                                Go to Dashboard
                            </Link>
                        </div>
